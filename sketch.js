...
function setup() {
  ...
  nRow = int(windowHeight/50); nCol = (displayWidth/50);
  ...
  spotNum = 8;
   for (let m = 0; m < spotNum; m++) {
    mySpotlight[m] = new Spotlight(random(displayWidth/2-50,displayWidth/2+50),random(windowHeight/2),200,random(-1,1),random(-1,1))
  }

   //initiating data for spotlight
  ansNum = 5;
   for (let h = 0; h < ansNum; h++) {
  myAnswer[h] = new Answer((h+1)*(displayWidth-200)/5, random(windowHeight*0.6,windowHeight*0.9), color(227,189,88), h);
   }
  textFont(font);
...
class Spotlight {
  constructor(x, y, diameter, xSpeed, ySpeed) {
    ...
    this.xSpeed = xSpeed;
    this.ySpeed = ySpeed;
    this.img = createGraphics(displayWidth,windowHeight);//creating another canvas on top
  }
